...
class DbOperation {
    ...
    function getUsersByParams($params) {
        ...
        $req = "SELECT * FROM USERS WHERE (UPPER(USERNAME) LIKE UPPER('%".$params['username']."%')) AND (UPPER(EMAIL) LIKE UPPER('%".$params['email']."%'))";
        
        
        //$req = "SELECT * FROM USERS WHERE (LCASE(username) = ?) OR (LCASE(email) = ?)";
        

        $stmt = $this->con->prepare($req);
        //$stmt->bind_param("ssss",$params['username'],$params['email']);
